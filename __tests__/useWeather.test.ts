...
describe('useWeather hook', () => {
    ...
    it('fetches forecast data successfully', async () => {
        ...
        await waitFor(() => expect(result.current.forecast.length).toBeGreaterThan(0));

        expect(result.current.forecast[0].minTemp).toBe(mockForecastData.list[0].main.temp_min);
        expect(result.current.forecast[0].maxTemp).toBe(mockForecastData.list[0].main.temp_max);
        expect(result.current.errorMessage).toBeNull();
    });
